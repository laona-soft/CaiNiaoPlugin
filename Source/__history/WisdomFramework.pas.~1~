...
uses
  ...
  {$ELSE}
  Windows, SysUtils, Classes, StrUtils, IniFiles,
  {$ENDIF}
  ResultCode, XmlConfig,
  {$IFDEF _USE_FOR_CPP}
...
{$IFDEF _USE_FOR_CPP}
  {$INCLUDE ClassImplementForC.inc}
{$ENDIF}
...
begin
  ...
  {$IFDEF _USE_FOR_CPP}
  DLLGetObjectForC := GetProcAddress(FDllHandle, 'GetCppObject');
  {$ENDIF}
...
{$IFDEF _USE_FOR_CPP}
  DLLGetPluginInfoForC := GetProcAddress(FDllHandle, 'GetPluginInfo');
{$ENDIF}
...
{$IFDEF _USE_FOR_CPP}
  DLLInitializePluginForC := GetProcAddress(FDllHandle, 'InitializePlugin');
{$ENDIF}
...
{$IFDEF _USE_FOR_CPP}
  DLLServiceStartForC := GetProcAddress(FDllHandle, 'ServiceStart');
{$ENDIF}
...
{$IFDEF _USE_FOR_CPP}
  DLLServiceStopForC := GetProcAddress(FDllHandle, 'ServiceStop');
{$ENDIF}
...
{$IFDEF _USE_FOR_CPP}
  DLLUninitializePluginForC := GetProcAddress(FDllHandle, 'UninitializePlugin');
{$ENDIF}
