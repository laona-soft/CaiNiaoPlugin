...
{$endif}
    InternalHttpServerRespList: TList;
    /// server main loop - don't change directly
    procedure Execute; override;
...
begin
  ...
  InternalHttpServerRespList := TList.Create;
{$ifdef USETHREADPOOL}
...
begin
  ...
  EnterCriticalSection(ProcessCS);
  try
    for i := 0 to InternalHttpServerRespList.Count-1 do
      THttpServerResp(InternalHttpServerRespList.List[i]).ResetServer(false);
    FreeAndNil(InternalHttpServerRespList);
  finally
...
begin
  ...
  EnterCriticalSection(fServer.ProcessCS);
  try
    fServer.InternalHttpServerRespList.Add(self);
  finally
...
begin
  ...
  try
    ...
    try
      ...
      if (fServer.InternalHttpServerRespList<>nil) and Internal then begin
        i := fServer.InternalHttpServerRespList.IndexOf(self);
        if i>=0 then
          fServer.InternalHttpServerRespList.Delete(i);
      end;
...
const
  // if body content length is bigger than 1 MB, a dedicated thread is used
  BIGBODYSIZE = 1024*1024;
...
begin
  ...
  repeat
    ...
    if IdemPChar(P,'VARY:') then
      Known := respVary else
      Known := reqUpgrade; // mark not found
    if Known<>reqUpgrade then
